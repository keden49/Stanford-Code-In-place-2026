...
# Lessons 
```python
random.random() #generates a random float value between 0 and 1
random.uniform(min,max)#generates random float value between the 2 values 
random.randint(min,max) #generates random integer between min and max inclusive 
random.seed() #provides a seed value which uses an algorithm to generate a value, this value is never really random, if random is initialized with a seed it will always generate the same sequence of numbers over and over again